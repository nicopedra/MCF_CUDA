...
//for evaluate forces with verlet list
int* primi_vicini;
int* quanti_primi_vicini;
...
//evaluate forces between particles
float Force(int, int,int);

//setting to 0 all distances accumulators
void re_start();

//update verlet list
void aggiorno_primi_vicini(); 

//check if update list
bool check_aggiornamento(); 
...
void Input(void){ //Prepare all stuff for the simulation
  ...
  dist = new float [npart];

  primi_vicini = new int [npart*npart];
  quanti_primi_vicini = new int [npart];
...
//Prepare array for measurements   //they're just indices
  ...
  //per controllare quando aggiornare la lista
  deltar = (rm-rcut)*0.5;

  cout << "distanza per cui riaggiornare: " << deltar << endl;
...
bool check_aggiornamento() {
	...
	float dr;
	for (int i=0;i<npart;i++) {
		dr = sqrt( (x[i]-xold[i])*(x[i]-xold[i]) + (y[i]-yold[i])*(y[i]-yold[i]) + (z[i]-zold[i])*(z[i]-zold[i]) );
		dist[i]+=dr;
		if (dist[i] > deltar)  {
			aggiornamento = true;
			re_start();
...
float Force(int ip, int idir,int offset){ //Compute forces as -Grad_ip V(r)
  float f=0.0;
  float dvec[3], dr;

  for (int i=0; i<quanti_primi_vicini[ip]; i++ ) {

      dvec[0] = Pbc( x[ip] - x[ primi_vicini[offset+i] ] );  // distance ip-i in pbc
      dvec[1] = Pbc( y[ip] - y[ primi_vicini[offset+i] ] );
      dvec[2] = Pbc( z[ip] - z[ primi_vicini[offset+i] ] );
...
// con la lista.. più efficiente l'altro
/*
...
//cycle over pairs of particles
  int offset = 0;
  for (int i=0; i<npart; i++) {
    for (int j = 0; j < quanti_primi_vicini[i]; j++) {

     dx = Pbc( x[i] - x[ primi_vicini[offset+j] ] );
     dy = Pbc( y[i] - y[ primi_vicini[offset+j] ] );
     dz = Pbc( z[i] - z[ primi_vicini[offset+j] ] );
...
// g(r)
  for (int i=0;i<nbins;i++) {
    ...
    stima_pot = v*0.5/(float)npart+vtail; //Potential energy per particle
    stima_kin = t/(float)npart; //Kinetic energy per particle
    stima_temp = (2.0 / 3.0) * t/(float)npart; //Temperature
    stima_etot = (t+v*0.5)/(float)npart+vtail; //Total energy per particle
    stima_press = rho*stima_temp+(w*0.5+ptail*(float)npart)/vol;
