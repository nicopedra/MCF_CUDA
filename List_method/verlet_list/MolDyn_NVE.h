...
float* dev_hist;// g(r)
//per capire se devo aggiornare
bool* dev_check;
//per monitorare le distanze
float* dist;
...
__constant__ float gpu_deltar; 
__constant__ float gpu_rm;

// simulation
int nstep, iprint, seed;
float delta;

//difference between rm and rcut
//control for updating list
float deltar;
...
__global__ void check_update(bool*,float*);

__global__ void update_list(bool*);

__global__ void force_gpu(float*,float*,float*,bool*);
...
void Input(Particles* P){ //Prepare all stuff for the simulation
  ...
  cout << "verlet r " << rm << endl; 
  rm = 1.2*rcut;
  
  deltar = (rm-rcut)*0.5;

  cout << "raggio di verlet per i primi vicini " << rm << endl;
  cout << "distanza minima per monitorare: " << deltar << endl;
  cudaSetDevice(1);
...
//same as above, but for the forces
 ...
 HANDLE_ERROR( cudaMalloc( (void**)&Nearest,
                              npart*npart*sizeof(bool) ) );
//allocate global memory on the gpu
 HANDLE_ERROR( cudaMalloc( (void**)&dist,
                              npart*sizeof(float) ) );
 ...
 HANDLE_ERROR( cudaMalloc( (void**)&dev_hist, nbins*2*sizeof(int)  ) );
 HANDLE_ERROR( cudaMalloc( (void**)&dev_check, sizeof(bool)  ) );
...
//copy configurations on the gpu
   ...
   HANDLE_ERROR( cudaMemcpy( P->vz, vz,
                              npart*sizeof(float),
                              cudaMemcpyHostToDevice ) );
 //metto i contenitori degli spostamenti a 0
  HANDLE_ERROR( cudaMemset(dist,0,npart*sizeof(float) ));

 HANDLE_ERROR( cudaMemset(Nearest,false,npart*npart*sizeof(bool) ));
 dim3 b(15,15);
 dim3 t(64,16);
 update_list<<<b,t>>>(Nearest);
...
//cuda memeset check false
//se tra loro cose che sono solo false.. allora ottengo false
//se c'è anche un sono true allora ottengo true

__global__ void check_update(bool* check, float* dist) {
	
	int i = threadIdx.x+blockIdx.x*blockDim.x;
	float dr;
	while (i<gpu_npart) {
		dr = sqrt( (tex1Dfetch(texx,i)-tex1Dfetch(texxold,i) )*( tex1Dfetch(texx,i)-tex1Dfetch(texxold,i)) +
		(tex1Dfetch(texy,i)-tex1Dfetch(texyold,i) ) *( tex1Dfetch(texy,i)-tex1Dfetch(texyold,i) )+
		(tex1Dfetch(texz,i)-tex1Dfetch(texzold,i) )*( tex1Dfetch(texz,i)-tex1Dfetch(texzold,i)) );
		dist[i] += dr;
		if (dist[i] > gpu_deltar) { 
			*check = true;
			return;
		}
...
//cuda memeset Nearest false
//is a symmetric matrix
__global__ void update_list(bool* Nearest) {
	float dvec[3];
	float dr;
	int tid;
	int ip = threadIdx.x + blockIdx.x*blockDim.x;
	while (ip < gpu_npart ) {
		tid = threadIdx.y + blockIdx.y*blockDim.y+ip+1;
		while( tid < gpu_npart ) {
			...
			if (dr<gpu_rm && dr>0) {
				Nearest[ip*gpu_npart+tid] = true;
				Nearest[tid*gpu_npart+ip] = Nearest[ip*gpu_npart+tid];
			}
			tid += blockDim.y*gridDim.y;
		}
...
void aggiornamento() {
	...
	HANDLE_ERROR( cudaMemset(dev_check,false, sizeof(bool) ));
        check_update<<<bl,th_kinetic>>>(dev_check,dist);
	bool check;
	HANDLE_ERROR( cudaMemcpy(&check,dev_check,sizeof(bool),cudaMemcpyDeviceToHost) );
	if (check) {
		HANDLE_ERROR( cudaMemset(Nearest,false,npart*npart*sizeof(bool) ));
		//cont_ag++;
...
void Move_gpu(Particles *P) {
 ...
 force_gpu<<<bl,th_verlet>>>(fx,fy,fz,Nearest);
...
__global__ void force_gpu(float*fx,float*fy,float*fz,bool* Nearest){
