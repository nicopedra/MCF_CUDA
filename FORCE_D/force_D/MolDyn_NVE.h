...
//funziona solo con 16 bl e 16 th se uso dim3 b() e dim3 t().. sbaglio qualcosa quando
//provo a scrivere la mia funzione Force usando blocchi in 2D
// però devo assolutamente trovare un modo per sfruttare le 2D dimensions dei blocchi e thread
...
float* Fz;
	...
	texture<float> texFx;//matrici della forza 
        texture<float> texFy;
	texture<float> texFz;
...
void Force_cpu(Particles*);  

void print_Force(); 
//################# FUNZIONI GLOBAL ########################
__global__ void Force(float*,float*,float*,float*,float*,float*); 
...
void Force_cpu(Particles* P) {

  float * FFx = new float [npart*npart];
  float * FFy = new float [npart*npart];
  float * FFz = new float [npart*npart];
   ...
   for (int i=0 ; i<npart ;i++) {
	for (int j=i;j<npart;j++) {
		...
		if (i==j or dr>=rcut)  {
			...
			FFx[i*npart+j] = dvec[0]*(48.0/pow(dr,14) - 24.0/pow(dr,8));
			FFy[i*npart+j] = dvec[1]*(48.0/pow(dr,14) - 24.0/pow(dr,8));
			FFz[i*npart+j] = dvec[2]*(48.0/pow(dr,14) - 24.0/pow(dr,8));
		}
		FFx[i*npart+j] = -FFx[j*npart+i];
		FFy[i*npart+j] = -FFy[j*npart+i];
		FFz[i*npart+j] = -FFz[j*npart+i];
	}
   }

   HANDLE_ERROR( cudaMemcpy( Fx, FFx,
                              npart*npart*sizeof(float),
                              cudaMemcpyHostToDevice ) ); 
   HANDLE_ERROR( cudaMemcpy( Fy, FFy,
                              npart*npart*sizeof(float),
                              cudaMemcpyHostToDevice ) );
   HANDLE_ERROR( cudaMemcpy( Fz, FFz,
                              npart*npart*sizeof(float),
                              cudaMemcpyHostToDevice ) );
...
float * FFz = new float [npart*npart];
  ...
  HANDLE_ERROR( cudaMemcpy(FFx,Fx,npart*npart*sizeof(float),cudaMemcpyDeviceToHost));
  HANDLE_ERROR( cudaMemcpy(FFy,Fy,npart*npart*sizeof(float),cudaMemcpyDeviceToHost));
  HANDLE_ERROR( cudaMemcpy(FFz,Fz,npart*npart*sizeof(float),cudaMemcpyDeviceToHost));
...
void exit(Particles* P) {
    ...
    cudaUnbindTexture( texFx );
    cudaUnbindTexture( texFy );
    cudaUnbindTexture( texFz );
...
void Move_gpu(Particles *P) {
 ...
 //Force_cpu(P);
 Force<<<b,t>>>(FFx,FFy,FFz,P->dev_x,P->dev_y,P->dev_z);
...
__global__ void Force(float*Fx,float*Fy,float*Fz,float*x,float*y,float*z) { //capire meglio successivamente come sfruttare l'anti simmetria
    ...
    while (i<gpu_npart) {
		j = threadIdx.y + blockIdx.y * blockDim.y+i;
		//j = threadIdx.x+i;
		while (j<gpu_npart) {
    		...
    		if (dr>0 && dr<gpu_rcut)  {
        		Fx[i*gpu_npart+j] = dvec[0]*(48.0/pow(dr,14) - 24.0/pow(dr,8));
        		Fy[i*gpu_npart+j] = dvec[1]*(48.0/pow(dr,14) - 24.0/pow(dr,8));
        		Fz[i*gpu_npart+j] = dvec[2]*(48.0/pow(dr,14) - 24.0/pow(dr,8));
    	        }
 		else {
			Fx[i*gpu_npart+j] = 0;
        		Fy[i*gpu_npart+j] = 0;
    	       ...
    	       Fx[j*gpu_npart+i] = -Fx[i*gpu_npart+j];
    	       Fy[i+j*gpu_npart] = -Fy[i*gpu_npart+j];
    	       Fz[i+j*gpu_npart] = -Fz[i*gpu_npart+j];
...
__global__ void verlet_gpu(float*xold,float*yold,float*zold,float*x,float*y,float*z,float*vx,float*vy,float*vz){
	...
	while (ip < gpu_npart ) {
		...
		temp2 =0;
		while( tid<gpu_npart ) {
				temp0 += tex1Dfetch(texFx,ip*gpu_npart+tid); 
				temp1 += tex1Dfetch(texFy,ip*gpu_npart+tid);
				temp2 += tex1Dfetch(texFz,ip*gpu_npart+tid);
			tid += blockDim.x;
		}
		f0[cacheIndex] = temp0;		
		f1[cacheIndex] = temp1;
...
__device__ float Pbc_gpu(float r) {  
