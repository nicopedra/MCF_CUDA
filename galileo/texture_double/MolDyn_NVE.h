...
__global__ void  measure_properties(double*,double*,int*);
...
__global__ void  measure_properties(double *v,double *w,int* hist) {//double* hist) {
...
void Measure(Particles* P) {
 ...
 Lock lock;
	...
	HANDLE_ERROR( cudaMemset(dev_hist,0, nbins*2*sizeof(int)  ) );
	...
	HANDLE_ERROR( cudaMemcpy(hist,dev_hist,nbins*2*sizeof(int),cudaMemcpyDeviceToHost) );

	for (int i=0;i<nbins;i++) {
	    deltaVr = rho*npart*4.*M_PI/3.*(pow((i+1)*bin_size,3)-pow((i)*bin_size,3));
	   gdir[i].push_back(double(hist[i])/deltaVr);
        }
