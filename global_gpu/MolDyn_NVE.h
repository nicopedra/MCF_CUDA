...
float m_temp;
float accettazione;
//for gdir and other physical quantities
float bin_size;
...
//calculate forces
 force_gpu<<<bl,th_verlet>>>(Fx,Fy,Fz,P->x,P->y,P->z);
//move particles with verlet-algorithm
 verlet_gpu<<<bl,th_verlet>>>(P->xold,P->yold,P->zold,P->x,P->y,P->z,P->vx,P->vy,P->vz,Fx,Fy,Fz);
...
void Measure(Particles* P) {
 ...
 Lock lock;
	...
	measure_properties<<<bl,th_measure>>>(lock,dev_v,dev_w,dev_hist,P->x,P->y,P->z);
	measure_kinetic<<<bl,th_kinetic>>>(lock,dev_k,P->vx,P->vy,P->vz);
