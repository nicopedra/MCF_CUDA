...
__global__ void verlet_gpu(float*,float*,float*,float*,float*,float*,float*,float*,float*,float,float,float,int);
...
__global__ void  measure_pot_virial(Lock lock,float*,float*,float*,float*,float*,float*,float,float,int,float);
__global__ void  measure_kinetic(Lock lock,float*,float*,float*,float*,float,float,int);
...
void Move_gpu(Particles *P) {
 ...
 verlet_gpu<<<bl,th>>>(P->dev_xold,P->dev_yold,P->dev_zold,P->dev_x,P->dev_y,P->dev_z,P->dev_vx,P->dev_vy,P->dev_vz,box,rcut,delta,npart);

 cudaDeviceSynchronize();//l'Host aspetta che la GPU abbia finito

}

__global__ void prova_gpu (float* a,float* p,float box) {
...
__global__ void verlet_gpu(float*xold,float*yold,float*zold,float*x,float*y,float*z,float*vx,float*vy,float*vz,float box,float rcut,float delta,int npart){//,float*fx,float*fy,float*fz) {
	...
	while (ip < npart ) {
		...
		temp2 =0;
		while( tid < npart ) {
			dvec[0] = Pbc_gpu(x[ip]-x[tid],box); 
			dvec[1] = Pbc_gpu(y[ip]-y[tid],box); 
			dvec[2] = Pbc_gpu(z[ip]-z[tid],box);
			dr=sqrt(dvec[0]*dvec[0]+dvec[1]*dvec[1]+dvec[2]*dvec[2]);
			if (dr<rcut && dr>0) {
	...
	if (cacheIndex == 0) {
		...
		xnew = Pbc_gpu( 2.0 * x[ip] - xold[ip] + f0[0] *delta*delta ,box);
		ynew = Pbc_gpu( 2.0 * y[ip] - yold[ip] + f1[0] *delta*delta ,box);
		znew = Pbc_gpu( 2.0 * z[ip] - zold[ip] + f2[0] *delta*delta ,box);

		vx[ip] = Pbc_gpu(xnew - xold[ip],box) / (2.0 * delta);
    		vy[ip] = Pbc_gpu(ynew - yold[ip],box) / (2.0 * delta);
    		vz[ip] = Pbc_gpu(znew - zold[ip],box) / (2.0 * delta);
...
__global__ void  measure_kinetic(Lock lock,float*vx,float*vy,float*vz,float *k,float box,float rcut,int npart) {
...
__global__ void  measure_pot_virial(Lock lock,float* x,float*y,float*z,float *v,float *w,float* hist,float box,float rcut,int npart,float bin_size) {
	...
	*v = 0.0;//potenziale	
	...
	while (i<npart-1) {
		...
		while (j<npart) {
			...
			dvec[0] = Pbc_gpu(x[i]-x[j],box);
                        dvec[1] = Pbc_gpu(y[i]-y[j],box);
                        dvec[2] = Pbc_gpu(z[i]-z[j],box);
                        dr=sqrt(dvec[0]*dvec[0]+dvec[1]*dvec[1]+dvec[2]*dvec[2]);
			bin = int(dr/bin_size);
...
void Measure(Particles* P) {
 ...
 Lock lock;
	...
	measure_kinetic<<<bl,th>>>(lock,P->dev_vx,P->dev_vy,P->dev_vz,dev_k,box,rcut,npart);

	measure_pot_virial<<<bl,th>>>(lock,P->dev_x,P->dev_y,P->dev_z,dev_v,dev_w,dev_hist,box,rcut,npart,bin_size);

	cudaDeviceSynchronize();//l'Host aspetta che il device abbia finito
